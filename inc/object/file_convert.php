...
class file_convert {
    ...
    <Placemark>
      ...
      ' . $track->get_kml_linestring() . '
            ...
            if ($coords != "") $output .= outputTask($coords);
        }
    ...
    static function OSGridToLatLong($gridRef) {
        ...
        return Array(toDeg($lat), toDeg($lon));
    }

    static function LatLongToOSGrid($p, $q) {
        $lat = file_convert::toRad($p);
        $lon = file_convert::toRad($q);
        $a = 6377563.396;
        $b = 6356256.910;
        $F0 = 0.9996012717;
        $lat0 = file_convert::toRad(49);
        $lon0 = file_convert::toRad(-2);
        $N0 = -100000;
        $E0 = 400000;
        $e2 = 1 - (($b * $b) / ($a * $a));
        $n = ($a - $b) / ($a + $b);
        ...
        return file_convert::gridrefNumToLet($E, $N, 6);
    }
    ...
    static function gridrefNumToLet($e, $n, $digits) {
        ...
        $gridRef = $letPair . file_convert::padLZ($e) . file_convert::padLZ($n);
        return $gridRef;
    }
    ...
    static function getCords($file, $cnum, $lookup, $isOR = 0) {
        ...
        $a = Array();
        if (!$isOR) {
            for ($i = $strt; $i < $strt + $cnum; $i++) {
                $c = explode(",", substr($file[$i], 10), 4);
                $a[$i - $strt] = LatLongToOSGrid($c[1], $c[0]);
                if ($out[0] != "") $out[0] = $out[0] . ";" . $a[$i - $strt];
                else $out[0] = $a[$i - $strt];
                $out[1][$i - $strt] = $a[$i - $strt];
            }
        } else {
            for ($i = $strt; $i < $strt + $cnum; $i++) {
                $c = explode(",", substr($file[$i], 10), 4);
            ...
            if (getDist(Array($out[1][0], $out[1][1])) <= 0.8) {
                $copy = $out[1][1];
                $out[1][1] = $out[1][2];
                $out[1][2] = $copy;
                $c = explode(";", $out[0]);
    ...
    static function getDist($cords) {
        ...
        for ($i = 0; $i < count($cords) - 1; $i++) {
            $p1 = gridrefLetToNum($cords[$i]);
            $p2 = gridrefLetToNum($cords[$i + 1]);
    ...
    static function Decimalize($a) { // Decimalizes a base 60 angle.
    ...
    function getCircleCords2($cords) {
        $cords[0] = file_convert::toRad($cords[0]);
        $cords[1] = file_convert::toRad($cords[1]);
        $out = "";
