...
class field_mlink extends _field_link {
    ...
    public function get_cms_list_wrapper($value, $object_class, $id): string {
        ...
        $select_options = ['where' => 'link.' . $module->primary_key . ' IS NOT NULL', 'join' => [$module->table_name . '_link_' . $source_module->table_name . ' link' => 'link_' . $source_module->primary_key . '= ' . $source_module->primary_key . ' AND link.' . $module->primary_key . '=' . $this->parent_form->{$module->primary_key}],//'order' => isset($this->order) ? $this->order : 'title'
        ];
