...
class parser {
    ...
    protected static $xht = array(
        '[bull /]' => '<big>&bull;</big>',
        '[copyright /]' => '&copy;',
        '[registered /]' => '&reg;',
        '[tm /]' => '<big>&trade;</big>');
    protected static $emoticons = array('confident', 'happy', 'crying', 'friendly', 'evil', 'panic', 'indifferent', 'angel', 'teasing', 'angry', 'polite', 'mad', 'tearing', 'yelling', 'sad', 'sceptical');
    protected static $altEmoticons = array(':)', ':D', ';(', ';)', '&gt;:)', ':O', ':|', 'O:)', ':P', ':(', ':3', ':X', ':@', ':()', ':/', ':\\');
    protected static $rpVersion = '1.0.1';
    protected static $rpAddr = 'www.bbcparser.recgr.com';
    ...
    public static function stir($seek, $replace, $subject) {
    ...
    public static function p($string, $toBr = 0, $justParse = 1, $useSmilies = 1, $simpleSmilies = 1, $protectMails = 1, $showFormattedTwice = 0, $onShutdown = "") {

        $s = ( string ) $string;
        if (empty ($s)) {
            echo "No text is availible to preview. \n";
            echo "<br /> \n";
            return;
        }
        if (PHP_VERSION <= 4) {
        ...
        if ($simpleSmilies) {
            foreach (self::$altEmoticons as $altEmNum => $altEmEnt) {
                $altEmEntParse = preg_quote($altEmEnt);
                $s = preg_replace("#([\n\r\t\040]+)$altEmEntParse([\n\r\t\040]+)#", "$1" . ':' . self::$emoticons [$altEmNum] . ':' . "$2", $s);
            }
        }
        if ($useSmilies) {
            $smiliesRepository = self::$emoticons;
            ...
            foreach ($smiliesRepository as $smilieKey => $smilieName) {
                ...
                $s = preg_replace("#([\n\r\t\040]+)$smilieNameParse([\n\r\t\040]+)#", "$1" . $smReplace . self::$emoticons [$smilieKey] . $smReplace2 . "$2", $s);
            }
        }
        // B a s i c	P a r s e	(M a i n)

        for ($b = 1; $b < count(self::$bbc); $b++) {
            $bbcn = '#' . preg_quote(self::$bbc [$b], '#') . "(.*)" . preg_quote(self::$bbc [$b + 1], '#') . '#Uis'; // needle

            $bbcr = self::$rep [$b] . "$1" . self::$rep [++$b]; // replacement

            $s = preg_replace($bbcn, $bbcr, $s);
        }
        foreach (self::$xht as $xhtBbc => $xhtHtml) {
            $s = self::stir($xhtBbc, $xhtHtml, $s);
        }
        ...
        if (preg_match("#\[list\](.*?)\[\/list\]#is", $s)) {
            ...
            $s = self::stir('[list]<ul', '<ul', $s);
            $s = self::stir('</ul>[/list]', '</ul>', $s);
        }
        // [list=xxx]...[/list] parse
        ...
        if ($protectMails != 0) {
            ...
            foreach ($correctMails as $mailNum => $mailContent) {
                $protected = self::protect($correctMails [$mailNum]);
